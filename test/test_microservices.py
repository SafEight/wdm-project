...
class TestMicroservices(unittest.TestCase):
    def test_stock(self):
        ...
        self.assertTrue("item_id" in item, msg=f"Item ID not in item dict {item}")

        item_id: str = item["item_id"]

        # Test /stock/find/<item_id>
        item: dict = tu.find_item(item_id)
        self.assertEqual(item["price"], 5, msg=f"Item ID not in item dict {item}")
        ...
        self.assertTrue(
            tu.status_code_is_failure(int(over_subtract_stock_response)),
            msg=f"Over subtracted stock {over_subtract_stock_response}",
        )
    ...
    def test_payment(self):
        ...
        self.assertTrue(
            tu.status_code_is_success(add_credit_response), msg=add_credit_response
        )
        ...
        payment_response = tu.payment_pay(user_id, 8, 10)
